  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold text-jj-grey dark:text-white">Messages</h1>
      </div>
      ...
      <div className="flex gap-4 h-[calc(100vh-220px)]">
              ...
              <li key={conversation.id}>
                <button
                  ...
                  className={
                    ...
                    (activeId === conversation.id ? ' bg-jj-neutral dark:bg-gray-800' : '')
                  }
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-sm font-medium text-jj-grey dark:text-white truncate">
                      {participantNames(conversation.id)}
                    </span>
                    <span className="text-xs text-jj-grey/50 dark:text-gray-500 whitespace-nowrap">
                      {conversation.last_message_at
                        ? new Date(conversation.last_message_at).toLocaleDateString()
                        : ''}
                    </span>
                  </div>
                  <p className="text-xs text-jj-grey/60 dark:text-gray-400 truncate mt-1">
        ...
        <div className="flex-1 rounded-xl border border-jj-grey/20 dark:border-gray-700 bg-white dark:bg-gray-900 flex flex-col overflow-hidden">
          {!activeConversation && (
            <div className="flex-1 flex items-center justify-center text-sm text-jj-grey/60 dark:text-gray-400">
          ...
          {activeConversation && (
            <>
              <div className="px-4 py-3 border-b border-jj-grey/10 dark:border-gray-800">
                <span className="text-sm font-semibold text-jj-grey dark:text-white">
                  {participantNames(activeConversation.id)}
                </span>
              </div>
              <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
                {messagesLoading && (
                  <p className="text-sm text-jj-grey/60 dark:text-gray-400">Loading messages...</p>
                )}
                {!messagesLoading && messages.length === 0 && (
                  <p className="text-sm text-jj-grey/60 dark:text-gray-400">No messages in this conversation.</p>
                )}
                {!messagesLoading && messages.map((message) => (
                  <div key={message.id} className="flex flex-col">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-medium text-jj-grey dark:text-white">
                        {senderName(message)}
                      </span>
                      <span className="text-xs text-jj-grey/40 dark:text-gray-500">
                        {message.sent_at ? new Date(message.sent_at).toLocaleString() : ''}
                      </span>
                    </div>
                    <p className="text-sm text-jj-grey/80 dark:text-gray-300 mt-0.5">
                      {message.body}
                    </p>
                  </div>
                ))}
              </div>
              <div className="px-4 py-3 border-t border-jj-grey/10 dark:border-gray-800 text-xs text-jj-grey/50 dark:text-gray-500">
